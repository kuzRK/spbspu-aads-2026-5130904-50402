...
template < class T >
sogdanov::Vector< T >::Vector(size_t k):
...
template < class T >
void sogdanov::Vector< T >::pushBackImpl(const T &val)
{
...
template < class T >
void sogdanov::Vector< T >::reserve(size_t pos, size_t count)
{
  assert(pos <= size_);
  if (count == 0) {
    return;
...
template < class T >
sogdanov::Vector< T >::Vector(Vector< T > &&rhs) noexcept:
  data_(rhs.data_),
  size_(rhs.size_),
...
template < class T >
sogdanov::Vector< T >::Vector(const Vector< T > &rhs):
  Vector(rhs.getSize())
{
  size_t constructed = 0;
  try {
...
template < class T >
sogdanov::Vector< T >::Vector(std::initializer_list< T > il):
  Vector< T >(il.size())
{
  size_t i = 0;
  try {
...
template < class T >
sogdanov::Vector< T > &sogdanov::Vector< T >::operator=(const Vector< T > &rhs)
{
...
template < class T >
sogdanov::Vector< T > &sogdanov::Vector< T >::operator=(Vector< T > &&rhs) noexcept
{
  if (this == std::addressof(rhs)) {
    return *this;
  }
...
template < class T >
T &sogdanov::Vector< T >::operator[](size_t id) noexcept
{
...
template < class T >
const T &sogdanov::Vector< T >::operator[](size_t id) const noexcept
{
  assert(id < getSize());
  return data_[id];
}
...
template < class T >
T &sogdanov::Vector< T >::at(size_t id)
{
  const Vector< T > *cthis = this;
...
template < class T >
const T &sogdanov::Vector< T >::at(size_t id) const
{
  if (id < getSize()) {
    return data_[id];
  }
...
template < class T >
sogdanov::VIter< T > sogdanov::Vector< T >::begin() noexcept
{
  return VIter< T >(this, 0);
}

template < class T >
sogdanov::VIter< T > sogdanov::Vector< T >::end() noexcept
{
  return VIter< T >(this, size_);
}

template < class T >
sogdanov::VCIter< T > sogdanov::Vector< T >::begin() const noexcept
{
  return VCIter< T >(this, 0);
}

template < class T >
sogdanov::VCIter< T > sogdanov::Vector< T >::end() const noexcept
{
  return VCIter< T >(this, size_);
}

template < class T >
size_t sogdanov::Vector< T >::getSize() const noexcept
{
  return size_;
}

template < class T >
size_t sogdanov::Vector< T >::getCapacity() const noexcept
{
  return capacity_;
}

template < class T >
bool sogdanov::Vector< T >::isEmpty() const noexcept
{
  return !size_;
}

template < class T >
void sogdanov::Vector< T >::pushBack(const T &val)
{
  if (size_ == capacity_) {
    size_t new_cap = capacity_ ? capacity_ * 2 : 1;
    T *new_data = static_cast< T * >(::operator new(sizeof(T) * new_cap));
...
template < class T >
void sogdanov::Vector< T >::pushFront(const T &val)
{
  if (size_ == capacity_) {
    size_t new_cap = capacity_ ? capacity_ * 2 : 1;
    T *new_data = static_cast< T * >(::operator new(sizeof(T) * new_cap));
...
template < class T >
void sogdanov::Vector< T >::popBack() noexcept
{
  assert(size_ > 0);
  data_[--size_].~T();
}
...
template < class T >
void sogdanov::Vector< T >::popFront()
{
  assert(size_ > 0);
  data_[0].~T();
...
template < class T >
void sogdanov::Vector< T >::swap(Vector< T > &rhs) noexcept
{
...
template < class T >
void sogdanov::Vector< T >::reserve(size_t cap)
{
  if (capacity_ >= cap) {
    return;
  }
...
template < class T >
void sogdanov::Vector< T >::shrinkToFit()
{
  if (capacity_ > size_) {
    if (size_ == 0) {
      for (size_t i = 0; i < size_; ++i) {
...
template < class T >
void sogdanov::Vector< T >::erase(size_t i)
{
  erase(i, i + 1);
}

template < class T >
void sogdanov::Vector< T >::erase(size_t beg, size_t end)
{
...
template < class T >
sogdanov::VIter< T > sogdanov::Vector< T >::erase(VIter< T > pos)
{
  size_t idx = pos.idx_;
  erase(idx);
...
template < class T >
sogdanov::VIter< T > sogdanov::Vector< T >::erase(VIter< T > first, VIter< T > last)
{
...
template < class T >
sogdanov::VIter< T > sogdanov::Vector< T >::erase(VCIter< T > pos)
{
  size_t idx = pos.idx_;
  erase(idx);
  return VIter< T >(this, idx);
...
template < class T >
void sogdanov::Vector< T >::insert(size_t i, const Vector< T > &rhs, size_t beg, size_t end)
{
...
template < class T >
void sogdanov::Vector< T >::insert(size_t i, const T &val)
{
  Vector< T > tmp;
  tmp.pushBack(val);
  insert(i, tmp, 0, 1);
...
template < class T >
template < class VecIt, class FwdIt >
void sogdanov::Vector< T >::insert(VecIt pos, FwdIt beg, FwdIt end)
{
  Vector< T > tmp;
...
template < class T >
sogdanov::VIter< T > sogdanov::Vector< T >::insert(VIter< T > pos, const T &val)
{
  size_t idx = pos.idx_;
  insert(idx, val);
  return VIter< T >(this, idx);
...
template < class T >
sogdanov::VIter< T > sogdanov::Vector< T >::insert(VIter< T > pos, const Vector< T > &other)
{
  size_t idx = pos.idx_;
...
template < class T >
sogdanov::VIter< T > sogdanov::Vector< T >::insert(size_t i, const Vector< T > &other)
{
...
template < class T >
template < class IT >
size_t sogdanov::Vector< T >::pushBackRange(IT begin, size_t k)
{
  if (k == 0) {
    return 0;
  }
