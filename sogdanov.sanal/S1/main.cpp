...
namespace sogdanov {
  ...
  void input(std::istream &in, List_pair &list)
  {
    ...
    while (in >> name) {
      ...
      std::pair< std::string, List< size_t > > entry{name, std::move(nums)};
      last = list_append(list, last, std::move(entry));
    }
  }
...
int main()
{
  sogdanov::List_pair list;
  sogdanov::input(std::cin, list);
  ...
  sogdanov::List< sogdanov::List< size_t > > transp;
  sogdanov::transpose(list, transp);

  try {
    sogdanov::output(std::cout, list, transp);
